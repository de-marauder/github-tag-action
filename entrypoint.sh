...
if [ -z "${default_branch}" ] && [ "$branch_history" == "full" ]; then
    echo "The DEFAULT_BRANCH should be autodetected when tag-action runs on on PRs else must be defined, See: https://github.com/de-marauder/github-tag-action/pull/230, since is not defined we find it natively"
...
if $pre_release; then
    ...
    if [ "$pre_tag_commit" == "$commit" ] && [ "$force_without_changes_pre" == "false" ]; then
